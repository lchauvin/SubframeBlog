...
- Nav: "THE LOG" and "ABOUT & RIG", Barlow Condensed 15px, `letter-spacing:.12em`, uppercase, gap 26px. Active item is `var(--color-text)` with a 2px `var(--color-accent)` bottom border (`padding-bottom:2px`); inactive is `var(--color-neutral-600)` with a transparent border. Then the search affordance: 1px `var(--color-divider)` box, `padding:5px 9px`, Lucide search 14px + "SEARCH" in mono 9.5px, `var(--color-neutral-600)`. It keeps that styling exactly but is a button — it opens the search overlay (also Cmd/Ctrl-K); below 720px it drops the "SEARCH" label and stays as the bare magnifier.
